...
const Recommandation = () => {
  ...
  const areDatesWithinFiveYears = (dob1, dob2) => {
      ...
      if (yearDiff > 5) {
          ...
          // If yearDiff is exactly 5, check the months and days to make sure it's within 5 years
                  ...
                  {memberRecommand ? memberRecommand.map(member => (
                      <div key={member.account_id} className="recommendation-item">
                          ...
                          {member.friendRequestReceive ? (
                              <div>
                                  <button onClick={() => { handleAcceptRequest(member.account_id, accountIdParm); fetchData() }}>Accept</button>
                                  <button onClick={() => { handleIgnoreRequest(member.account_id, accountIdParm); fetchData() }}>Ignore</button>
                              </div>
                          ) : (
                              <button onClick={() => { handleSendRequest(accountIdParm, member.account_id); fetchData() }}>Send Friend Request</button>
                          )}
                      </div>
                  ))
                  :
